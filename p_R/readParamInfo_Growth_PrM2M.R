#--read Growth_PrM2M info
#' @title Read Growth_PrM2M info
...
#' if (FALSE){
#'   conn="inputSpecs_Growth_PrM2M.data_vertical.txt";
#'   lstResults = readParamInfo_Growth_PrM2M(conn,verbose=TRUE);
#' }
#'
#' # example code reading "horizontal" data format
#' if (FALSE){
#'   conn="inputSpecs_Growth_PrM2M.data_horizontal.txt";
#'   lstResults = readParamInfo_Growth_PrM2M(conn,verbose=TRUE);
#' }
#'
#' # example code reading "function" format
#' if (FALSE){
#'   conn="inputSpecs_Growth_PrM2M.function.txt";
#'   lstResults = readParamInfo_Growth_PrM2M(conn,verbose=TRUE);
#' }
#'
#' @import dplyr
...
#' @export
#'
readParamInfo_Growth_PrM2M<-function(conn,verbose=FALSE){
  lns = purrr::keep(stringr::str_trim(readLines(conn,skipNul=TRUE)),
                    \(x)stringr::str_length(x)>0) |>
          extractLines(start="GROWTH_PrM2M",end="END");
...
if (FALSE){
  ...
  conn=file.path(dirPrj,"testing/testGrowth/inputSpecs_Growth_PrM2M.data-vertical.txt");
  resV = readParamInfo_Growth_PrM2M(conn,TRUE);
  View(resV$dfr);
}
...
if (FALSE){
  ...
  source(file.path(dirPrj,"R/MiscFunctions_Text.R"))
  source(file.path(dirPrj,"R/readParamInfoSectionType1.R"))
  conn=file.path(dirPrj,"testing/testGrowth/inputSpecs_Growth_PrM2M.data-horizontal.txt");
  resH = readParamInfo_Growth_PrM2M(conn,TRUE);
  View(resH$dfr);
}
...
if (FALSE){
  ...
  source(file.path(dirPrj,"R/MiscFunctions_Text.R"))
  source(file.path(dirPrj,"R/readParamInfoSectionType1.R"))
  conn=file.path(dirPrj,"testing/testGrowth/inputSpecs_Growth_PrM2M.function.txt");
  resF = readParamInfo_Growth_PrM2M(conn,TRUE);
  View(resF$dfr);
}
