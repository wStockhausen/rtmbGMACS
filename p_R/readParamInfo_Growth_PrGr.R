...
#' if (FALSE){
#'   conn="inputSpecs_Growth_PrGr.data_vertical.txt";
#'   lstResults = readParamInfo_Growth_PrGr(conn,verbose=TRUE);
#' }
#'
#' # example code reading "horizontal" data format
#' if (FALSE){
#'   conn="inputSpecs_Growth_PrGr.data_horizontal.txt";
#'   lstResults = readParamInfo_Growth_PrGr(conn,verbose=TRUE);
#' }
#'
#' # example code reading "function" format
#' if (FALSE){
#'   conn="inputSpecs_Growth_PrGr.function.txt";
#'   lstResults = readParamInfo_Growth_PrGr(conn,verbose=TRUE);
#' }
#'
#' @import dplyr
...
#' @export
#'
readParamInfo_Growth_PrGr<-function(conn,verbose=FALSE){
  lns = purrr::keep(stringr::str_trim(readLines(conn,skipNul=TRUE)),
                    \(x)stringr::str_length(x)>0) |>
          extractLines(start="GROWTH_PrGr",end="END");
