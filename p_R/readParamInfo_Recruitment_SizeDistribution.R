#--read Recruitment_SizeDistribution info
#' @title Read Recruitment_SizeDistribution info
#' @description Function to read recruitment size distribution info.
...
#' @export
#'
readParamInfo_Recruitment_SizeDistribution<-function(conn,verbose=FALSE){
  lns = purrr::keep(stringr::str_trim(readLines(conn,skipNul=TRUE)),
                    \(x)stringr::str_length(x)>0) |>
          extractLines(start="RECRUITMENT_SIZE_DISTRIBUTION",end="END");
...
if (FALSE){
  ...
  resV = readParamInfo_Recruitment_SizeDistribution(conn,TRUE);
  View(resV$dfr);
}
...
if (FALSE){
  ...
  resH = readParamInfo_Recruitment_SizeDistribution(conn,TRUE);
  View(resH$dfr);
}
...
if (FALSE){
  ...
  resF = readParamInfo_Recruitment_SizeDistribution(conn,TRUE);
  View(resF$dfr);
}
