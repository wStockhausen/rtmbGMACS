...
lstCTL = readFile_CTL(fn=file.path(dirThs,"CTL_Allometry1.txt"));

##--determine model matrices for conversion from actual RTMB parameters to model parameters
source(file.path(dirPrj,"R/determineModelMatrices.R"))
lstModMtx = determineModelMatrices(lstCTL$dfrFcnParInfo);
...
source(file.path(dirPrj,"R/determineInitialValues.R"))
lstInfoIVs = determineInitialValues(lstCTL,lstModMtx);
...
source(file.path(dirPrj,"R/mapFunctionsToData_Allometry.R"))
dfrZWppp = mapFunctionsToData_Allometry(dfrZW1,lstCTL$dfrFcnsExp,lstModMtx);
...
inputs = list(lstAllom=lstAllom,
                lstModMtx=lstModMtx,
                #nparsFEs=idxFEs, nparsECs=idxECs, nparsREs=idxREs,
                lst_pinfo=lstInfoIVs$lst_pinfo,
                lst_map=lstInfoIVs$lst_map,
                lst_priors=lstInfoIVs$lst_priors);#--list will be expanded in complete model objective function with info for other processes
...
plt = ggplot(dfrZWppp |> dplyr::select(z,obs,x),aes(x=z,y=obs,colour=x)) + geom_point();
print(plt);
