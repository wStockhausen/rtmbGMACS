...
str<-paste0('
  ...
  CODE
  mfAll  = dmsYSC;
  mf2024 = dmsYSC |> dplyr::filter(y=="2024",x=="male");
  contr_none = "contr.none";#--no contrasts
  contr_sum  = list(x="contr.sum",m="contr.sum");
  END_CODE
  FUNCTIONS
  fcn_idx  fcn_id     function  frame    params              vars    eqsRE covTypeRE linkRE    description 
    1      pwrLaw1     pwrLaw1   mfAll   pA,pB1                z      NA       NA     NA       w(z)_=_pA_*_(z^pB1)
    2      pwrLaw2     pwrLaw2   mf2024  pLnA,pLnS,pB2,pZ0     z      NA       NA     NA       w(z)_=_exp(pLnA+pLnS+pB2*ln(z/pZ0))
  END_FUNCTIONS
  PARAM_EQS
  par_idx  fcn_idx  par_id  link_fcn  feEQs  feContrasts  ecEQs  reEQs  reCovType
  1         1       pA      ident     ~1+x*m  contr_sum   NA     NA     NA
  2         1       pB1     ident     ~0+x:m  contr_none  NA     NA     NA
  3         2       pLnA    ident     ~1+x    contr_none  NA     NA     NA
  4         2       pLnS    ident     ~1+x    contr_none  NA     NA     NA
  5         2       pB2     ident     ~1+x    contr_none  NA     NA     NA
  6         2       pZ0     ident     ~1+x    contr_none  NA     NA     NA
END_PARAM_EQS
  FE_VALS
  pA
...
for (par_idx_ in tbl$par_idx){
  ...
  lstModMtx[[rw$par_id]] = list(FEs=lstModMtxFEs,ECs=lstModMtxECs,REs=lstModMtxREs,links=getLinkFcn(rw$link_fcn));
...
#     parname = paste0("allom_EC_",par_id_);
#     dfr = lstECVs[[par_id_]];
...
#     parname = paste0("allom_RE_",par_id_);
#     dfr = lstREVs[[par_id_]];
...
if (length(lstFEVs)>0){
  for (par_id_ in names(lstFEVs)){
    ...
    parname = paste0("allom_FE_",par_id_);
    dfrIV   = lstFEVs[[par_id_]];               #--dataframe with model parameter info
    lst_map = c(lst_map,createParamsMap(dfrIV));#--create RTMB "map" for model parameters
    vIVs    = dfrIV$IV;                         #--initial values for model parameters
    lst_priors[[parname]] = getPriorInfo(dfrIV);
    ##--reduced model matrix
    T = as.matrix(lstAllom$lstModMtx[[par_id_]]$FEs$mtxRedMM);
    ##--get SVD of the reduced model matrix
    lst = svd(as.matrix(T,nu=nrow(T),nv=ncol(T)));
    ##--get link and inverse link functions defining parameter transformation
    links = lstAllom$lstModMtx[[par_id_]]$links;
    ##--calculate initial values of "actual" RTMB parameters
    vP = (lst$v %*% diag(1/lst$d) %*% t(lst$u)) %*% links$link(vIVs);
    ##--recalculate initial values of model parameters as check
    vpIVs = links$link_inv(T %*% vP);
    cat(par_id_,": ",lstModMtx[[par_id_]]$FEs$f,"\n");
    cat("  actual initial values:",vP,"\n");
    cat("original initial values:",vIVs,"\n");
    cat(" checked initial values:",vpIVs,"\n\n");
...
str<-paste0('
  ...
  END_FRAMES
  FUNCTIONS
  fcn_idx  id        function  frame   params             vars  reEQs reCovType reLink    description 
    1    pwrLaw1   pwrLaw1   mfAll   pA,pB1                z      NA      NA     NA       w(z)_=_pA_*_(z^pB)
    2    pwrLaw2   pwrLaw2   mf2024  pLnA,pLnS,pB2,pZ0     z      NA      NA     NA       w(z)_=_exp(pLnA+pLnS+pB*ln(z/pZ0))
  END_FUNCTIONS
  PARAM_EQS
  par_idx  fcn_idx  par_id  link_fcn  feEQs  feContrasts  ecEQs  reEQs  reCovType
  1         1       pA      ident     ~0+x*m  contr_none  NA     NA     NA
  2         1       pB1     ident     ~0+x:m  contr_none  NA     NA     NA
  3         2       pLnA    ident     ~0+x    contr_none  NA     NA     NA
  4         2       pLnS    ident     ~0+x    contr_none  NA     NA     NA
  5         2       pB2     ident     ~0+x    contr_none  NA     NA     NA
  6         2       pZ0     ident     ~0+x    contr_none  NA     NA     NA
  END_PARAM_EQS
  FE_VALS
  pA
