...
#' @param info - info list (output list from [extractParamInfo_FisherRates()])
...
#' @export
#'
calcFisheriesRates<-function(dims,info,params,lstSels,verbose=FALSE){
  if (verbose) cat("Starting calcFisheriesRates.\n")
  lstSrvVals<-list();#--arrays of expanded fishery rates, by fleet
  for (flt_ in info$flts){
    lstSrvVals[[flt_]] = RTMB::AD(array(0,c(dims$nYs,dims$nSs,dims$nCs,nTypes)));
  }
