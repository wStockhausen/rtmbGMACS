#--extract Recruitment_SizeDistribution parameters
#'
#' @title Extract Recruitment_SizeDistribution parameters from parameter info list
#' @description Function to extract Recruitment_SizeDistribution parameters from parameter info list.
#' @param lst - parameter info list from [readParamInfo_Recruitment_SizeDistribution()]
...
#' @export
#'
extractParamInfo_Recruitment_SizeDistribution<-function(lst,
                                            dims=NULL,
                                            verbose=TRUE){
  if (verbose) message("starting extractParameters_Recruitment_SizeDistribution.")
  if (FALSE){
    ...
    verbose = TRUE;
    lst = res;#--assumed output from `res  = readParamInfo_Recruitment_SizeDistribution(conn,verbose=FALSE);`
  }

  #--expand Recruitment_SizeDistribution parameter information----
  if (tolower(lst$option)=="function"){
    ...
    if (is.character(lst$dfr$value[1])){
      ...
      if (verbose){
        message("in extractParamInfo_Recruitment_SizeDistribution: lst$dfr:")
        print(lst$dfr);
      }
    ...
    if (verbose) {
      message("in extractParameters_Recruitment_SizeDistribution: resolved 'data' option values");
      print(dfr);
    }
    pPrM = dfr$IV;#--probability of undergoing a molt
    map = list(pPrM=factor(NA+pPrM));#--NAs indicate fixed values
    if (verbose) message("in extractParameters_Recruitment_SizeDistribution: expanding dataframe.")
...
if (FALSE){
  ...
  res1v  = readParamInfo_Recruitment_SizeDistribution(conn,verbose=FALSE);
  res2v = extractParamInfo_Recruitment_SizeDistribution(res1v,dims,verbose=FALSE);
}
...
if (FALSE){
  ...
  res1h  = readParamInfo_Recruitment_SizeDistribution(conn,verbose=FALSE);
  res2h = extractParamInfo_Recruitment_SizeDistribution(res1h,dims,verbose=FALSE);
}
...
if (FALSE){
  ...
  res1f  = readParamInfo_Recruitment_SizeDistribution(conn,verbose=FALSE);
  res2f = extractParamInfo_Recruitment_SizeDistribution(res1f,dims,verbose=TRUE);
}
