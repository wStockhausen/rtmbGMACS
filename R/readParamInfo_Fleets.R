#--read Fleets info
#' @title Read Fleets info
#' @description Function to read fleets parameterization info info.
...
#' @export
#'
readParamInfo_Fleets<-function(conn,verbose=FALSE){
...
if (FALSE){
  ...
  conn=file.path(dirPrj,"testing/testFleets/inputSpecs_Fleets.data-vertical.txt");
  resV = readParamInfo_Fleets(conn,TRUE);
  View(resV$dfr);
}
...
if (FALSE){
  ...
  source(file.path(dirPrj,"R/MiscFunctions_Text.R"))
  source(file.path(dirPrj,"R/readParamInfoSectionType1.R"))
  conn=file.path(dirPrj,"testing/testFleets/inputSpecs_Fleets.data-horizontal.txt");
  resH = readParamInfo_Fleets(conn,TRUE);
  View(resH$dfr);
}
...
if (FALSE){
  ...
  source(file.path(dirPrj,"R/MiscFunctions_Text.R"))
  source(file.path(dirPrj,"R/readParamInfoSectionType1.R"))
  conn=file.path(dirPrj,"testing/testFleets/inputSpecs_Fleets.function.txt");
  resF = readParamInfo_Fleets(conn,TRUE);
  View(resF$Fcns);
  View(resF$MPs);
  View(resF$OPs);
  View(resF$DPs);
