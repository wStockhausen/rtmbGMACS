#--extract Growth_PrMolt parameters
#'
#' @title Extract Growth_PrMolt parameters from parameter info list
#' @description Function to extract Growth_PrMolt parameters from parameter info list.
#' @param lst - parameter info list from [readParamInfo_Growth_PrMolt()]
...
extractParamInfo_Growth_PrMolt<-function(lst,
  ...
  if (FALSE){
    ...
    verbose = TRUE;
    lst = res;#--assumed output from `res  = readParamInfo_Growth_PrMolt(conn,verbose=FALSE);`
  }

  #--expand Growth_PrMolt parameter information----
  if (tolower(lst$option)=="function"){
    ##--option == "function"----
    ##--inputs are functions and parameters definitions
    out = extractParamInfoFunctionType1(lst,dims$dmsYSC,"Growth_PrMolt",verbose=verbose);
    ...
    if (is.character(lst$dfr$value[1])){
      ...
      if (verbose){
        message("in extractParamInfo_Growth_PrMolt: lst$dfr:")
        print(lst$dfr);
      }
...
if (FALSE){
  ...
  res1v  = readParamInfo_Growth_PrMolt(conn,verbose=FALSE);
  res2v = extractParamInfo_Growth_PrMolt(res1v,dims,verbose=FALSE);
}
...
if (FALSE){
  ...
  res1h  = readParamInfo_Growth_PrMolt(conn,verbose=FALSE);
  res2h = extractParamInfo_Growth_PrMolt(res1h,dims,verbose=FALSE);
}
...
if (FALSE){
  ...
  res1f  = readParamInfo_Growth_PrMolt(conn,verbose=FALSE);
  res2f = extractParamInfo_Growth_PrMolt(res1f,dims,verbose=TRUE);
}
