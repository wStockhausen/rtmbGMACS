...
#' @title Precision (or var-covar) matrix for an AR1 process
#' @description Function for creating a precision or var-covar matrix for
#' an AR1 process
...
#' @param vcov bool If the var-covar matrix should be returned instead of the
#' precision matrix.
#'
#' @return Q.AR1 returns either a precision or variance-covariance matrix
...
Q.AR1 <- function(M, sigma, rho, sparse=TRUE, vcov=FALSE){
    ...
    Q[M,M] <- 1.
    Q <- Matrix::Diagonal(M, 1 / sigma) %*% Q %*% Matrix::Diagonal(M, 1 / sigma);
