#--extract Recruitment_TimeSeries parameters
#'
#' @title Extract Recruitment_TimeSeries parameters from parameter info list
#' @description Function to extract Recruitment_TimeSeries parameters from parameter info list.
#' @param lst - parameter info list from [readParamInfo_Recruitment_TimeSeries()]
...
#' @export
#'
extractParamInfo_Recruitment_TimeSeries<-function(lst,
                                            dims=NULL,
                                            verbose=TRUE){
  if (verbose) message("starting extractParameters_Recruitment_TimeSeries.")
  if (FALSE){
    ...
    verbose = TRUE;
    lst = res;#--assumed output from `res  = readParamInfo_Recruitment_TimeSeries(conn,verbose=FALSE);`
  }

  #--expand Recruitment_TimeSeries parameter information----
  if (tolower(lst$option)=="function"){
    ...
    if (is.character(lst$dfr$value[1])){
      ...
      if (verbose){
        message("in extractParamInfo_Recruitment_TimeSeries: lst$dfr:")
        print(lst$dfr);
      }
    ...
    if (verbose) {
      message("in extractParameters_Recruitment_TimeSeries: resolved 'data' option values");
      print(dfr);
    }
    pPrM = dfr$IV;#--probability of undergoing a molt
    map = list(pPrM=factor(NA+pPrM));#--NAs indicate fixed values
    if (verbose) message("in extractParameters_Recruitment_TimeSeries: expanding dataframe.")
...
if (FALSE){
  ...
  res1v  = readParamInfo_Recruitment_TimeSeries(conn,verbose=FALSE);
  res2v = extractParamInfo_Recruitment_TimeSeries(res1v,dims,verbose=FALSE);
}
...
if (FALSE){
  ...
  res1h  = readParamInfo_Recruitment_TimeSeries(conn,verbose=FALSE);
  res2h = extractParamInfo_Recruitment_TimeSeries(res1h,dims,verbose=FALSE);
}
...
if (FALSE){
  ...
  res1f  = readParamInfo_Recruitment_TimeSeries(conn,verbose=FALSE);
  res2f = extractParamInfo_Recruitment_TimeSeries(res1f,dims,verbose=TRUE);
}
