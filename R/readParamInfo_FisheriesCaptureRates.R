#--read fisheries capture rates info
#' @title Read fisheries capture rates info
...
#' @export
#'
readParamInfo_FisheriesCaptureRates<-function(conn,verbose=FALSE){
  lns = purrr::keep(stringr::str_trim(readLines(conn,skipNul=TRUE)),
                    \(x)stringr::str_length(x)>0) |>
          extractLines(start="FISHERIES_CAPTURE_RATES",end="END");
...
if (FALSE){
  ...
  resV = readParamInfo_FisheriesCaptureRates(conn,TRUE);
  View(resV$dfr);
}
...
if (FALSE){
  ...
  resH = readParamInfo_FisheriesCaptureRates(conn,TRUE);
  View(resH$dfr);
}
...
if (FALSE){
  ...
  resF = readParamInfo_FisheriesCaptureRates(conn,TRUE);
  View(resF$Fcns);
  View(resF$MPs);
  View(resF$OPs);
  View(resF$DPs);
