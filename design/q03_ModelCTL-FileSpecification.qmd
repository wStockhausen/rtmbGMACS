---
title: "rtmbGMACS: CTL file specification"
author:
...
## CTL file {#sec-CTL-File-}

The CTL file is the "control" file used to define the order of model operations, process-level functions, 
and model parameters. The CTL file can be divided into the following sections:
...
A text-based CTL file can be read into R using the `readFile_CTL` function. Alternatively, the individual 
sections of a complete CTL file constitute an `R` list, which can be (in whole or in specific parts) be defined 
in `R`. Thus, the term "file" is used rather loosely here.

### Model Dimensions {#sec-CTL-File-Dimensions}

The following code sets up the model dimensions for a small model which will serve as the basis for further example code:

```{r}
#| label: CTL-File-ModelDimensions
#| echo: true
vRs="EBS";
...
The model tracks annual sex-specific abundance/biomass through time using two maturity categories, three post-molt age categories, 
and category-specific sizes. The resulting `DimsMap` for the stock categories is given in @tbl-dmsC. 

```{r}
#| label: tbl-dmsC
#| tbl-cap: "First 60 rows of the stock `DimsMap` used to create stock-level matrices."
#| eval: !expr '(require(kableExtra))'
head(dmsC,n=60) |> kableExtra::kbl(booktabs=TRUE,longtable=TRUE) |> kableExtra::column_spec(width);
```

Annual abundance is projected through time using `r length(ss)` seasons.
