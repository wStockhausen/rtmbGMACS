...
str<-paste0('
  ...
  END_CODE
  FUNCTIONS
  fcn_idx  fcn_id     function  frame    params              vars    reEQs reCovType reLink    description 
  ...
  END_FUNCTIONS
  PARAM_EQS
  par_idx  fcn_idx par_id  feEQs   feCons       feLink    ecEQs  ecLink   reEQs reCovType reLink   
...
if (length(lstFEVs)>0){
  for (par_id_ in names(lstFEVs)){
    #--testing: par_id_ = names(lstFEVs)[1];
    parname = paste0("allom_FE_",par_id_);
    dfrIV = lstFEVs[[par_id_]];
    vIVs = dfrIV$IV;#--initial values
    T = as.matrix(lstModMtx$pA$FEs$ModMtx);
    lst = svd(as.matrix(T,nu=nrow(T),nv=ncol(T)));
    vP = (lst$v %*% diag(1/lst$d) %*% t(lst$u)) %*% vIVs;
    vpIVs = T %*% vP;
  }
}
...
str<-paste0('
  ...
  END_FUNCTIONS
  PARAM_EQS
  par_idx  fcn_idx  id    eEQs    feCons        feLink   ecEQs  ecLink   reEQs reCovType reLink  
