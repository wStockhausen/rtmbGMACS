...
if (type=="data-vertical"){
  ...
  lstNM = extractParamInfo_Growth_PrM2M(res,dims,FALSE);
...
if (type=="data-horizontal"){
  ...
  res   = readParamInfo_Growth_PrM2M(conn,TRUE);
  lstNM = extractParamInfo_Growth_PrM2M(res,dims);
  params = list(pNM_FPs=lstNM$params);#--"FP" for "fixed" parameters
} else
if (type=="function"){
...
#--test wAtZ function----
source(file.path(dirPrj,"R/calcGrowth_PrM2M.R"));
M = calcGrowth_PrM2M(inputs$dims,inputs$lstNM,params,FALSE,loopIC_=TRUE); #--slower
M = calcGrowth_PrM2M(inputs$dims,inputs$lstNM,params,FALSE,loopIC_=FALSE);#--faster
...
obj_fun<-function(params){
  ...
  info = inputs$lstNM;
  M = calcGrowth_PrM2M(dims,info,params,verbose);
  REPORT(M);
