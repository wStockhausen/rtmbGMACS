#--test "pre-specified" fishery rates specifications for the gmacs objective function
require(RTMB);
require(ggplot2);
...
if (FALSE){
  ...
  source(file.path(dirPrj,"R","Functions_FisheriesRates.R"))
...
obj_fun<-function(params){
  ...
  info = inputs$lstSrv;
  lstSrv = calcFisheriesRates(dims,info,params,input$lstSel,verbose);
  REPORT(lstSrv);
