...
Generalized Model for Assessing Crustacean Stocks ([GMACS](`r hl_GMACS`)) model that was developed 
using AD MODEL BUILDER ([ADMB](`r hl_ADMB`)). RTMB ([R Bindings for Template Model Builder](`r hl_RTMB`)) is 
an R package that provides a native R interface to Template Model Builder ([TMB](`r hl_TMB`)), an R package that allows the user to develop and run C++-based statistical models (similar to ADMB) using automatic differentiation in the R framework.
...
The purpose of rtmbGMACS is to implement a population model for a fished 
...
Here, the RTMB model is referred to as "gmacs" while the R package is referred to "rtmbGMACS". The 
...
The rtmbGMACS *R* package allows 
